...
@dataclasses.dataclass(frozen=True)
class Aloha_Rebar_Inputs(transforms.DataTransformFn):
    """Inputs for the Aloha policy.
    ...
    # The expected cameras names. All input cameras must be in this set. Missing cameras will be
    # replaced with black images and the corresponding `image_mask` will be set to False.
    EXPECTED_CAMERAS: ClassVar[tuple[str, ...]] = ("gopro_0", "webcam_1", "webcam_2", "cam_right_wrist")
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # Add the extra images.
        extra_image_names = {
            "webcam_1_rgb": "webcam_1",
            "webcam_2_rgb": "webcam_2",
        }
        for dest, source in extra_image_names.items():
            if source in in_images:
                images[dest] = in_images[source]
...
@dataclasses.dataclass(frozen=True)
class Aloha_Rebar_Outputs(transforms.DataTransformFn):
    """Outputs for the Aloha policy."""

    # If true, this will convert the joint and gripper values from the standard Aloha space to
    # the space used by the pi internal runtime which was used to train the base model.
    adapt_to_pi: bool = False #-True
